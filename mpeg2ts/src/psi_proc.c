...
typedef struct psi_table_proc_ctx_s {
	...
	/**
	 * Processor's table mutual exclusion lock
	 */
	pthread_mutex_t psi_table_ctx_mutex;
	/**
	 * MPEG2-TS Packet IDentifier (PID) for this elementary stream.
	 */
	uint16_t pid;
	/**
		...
		const char *settings_str, log_ctx_t *log_ctx, va_list arg)
{
	...
	ret_code= pthread_mutex_init(&psi_table_proc_ctx->psi_table_ctx_mutex,
			NULL);
	CHECK_DO(ret_code== 0, goto end);

	psi_table_proc_ctx->tscc_input= TS_CC_UNDEF;

	psi_table_proc_ctx->pid= TS_MAX_PID_VAL+ 1; // Initialize to invalid value
...
static void psi_table_proc_close(proc_ctx_t **ref_proc_ctx)
{
	...
	/* Release mutex */
	pthread_mutex_destroy(&psi_table_proc_ctx->psi_table_ctx_mutex);
		...
		fifo_ctx_t* iput_fifo_ctx, fifo_ctx_t* oput_fifo_ctx)
{
	...
	pid= psi_table_proc_ctx->pid;
	psi_table_ctx_mutex_p= &psi_table_proc_ctx->psi_table_ctx_mutex;

	/* Get next table from input stream */
	ret_code= psi_table_dec_get_next_table(iput_fifo_ctx, LOG_CTX_GET(),
			&psi_table_proc_ctx->tscc_input, pid, &psi_table_ctx);
	...
	if(new_table_version_found!= 0) {
		...
		LOGEV("New %s table parsed: PID= %u (0x%0x); version %u (0x%0x)\n",
				pid== 0? "PAT": "PSI",
		...
		const proc_if_rest_fmt_t rest_fmt, void **ref_reponse)
{
	...
	psi_table_proc_ctx_t *psi_table_proc_ctx= NULL;  // Do no release
		...
		const proc_frame_ctx_t *proc_frame_ctx)
{
	...
	/* Update PID value (actually should never change) */
	((psi_table_proc_ctx_t*)proc_ctx)->pid= proc_frame_ctx->es_id;

...
ts_ctx_t* ts_ctx_dup(const ts_ctx_t* ts_ctx_arg)
{
	...
	if(payload_size> 0) {
		...
		payload_cpy= (uint8_t*)malloc(payload_size);
		CHECK_DO(payload_cpy!= NULL, goto end);
		memcpy(payload_cpy, payload, payload_size);
		ts_ctx->payload= payload_cpy;
		payload_cpy= NULL; // Avoid double referencing
	}
...
void ts_ctx_release(ts_ctx_t **ref_ts_ctx)
{
	...
	if(ref_ts_ctx== NULL || (ts_ctx= *ref_ts_ctx)== NULL)
		return;
...
ts_af_ctx_t* ts_af_ctx_dup(const ts_af_ctx_t* ts_af_ctx_arg)
{
	...
	if(af_remaining_size> 0) {
		...
		af_remaining_cpy= (uint8_t*)malloc(af_remaining_size);
		CHECK_DO(af_remaining_cpy!= NULL, goto end);
		memcpy(af_remaining_cpy, af_remaining, af_remaining_size);
		ts_af_ctx->af_remaining= af_remaining_cpy;
		af_remaining_cpy= NULL; // Avoid double referencing
	}

	end_code= STAT_SUCCESS;
end:
	if(af_remaining_cpy!= NULL)
		free(af_remaining_cpy);
	if(end_code!= STAT_SUCCESS)
...
void ts_af_ctx_release(ts_af_ctx_t **ref_ts_af_ctx)
{
	...
	if(ref_ts_af_ctx== NULL || (ts_af_ctx= *ref_ts_af_ctx)== NULL)
		return;
